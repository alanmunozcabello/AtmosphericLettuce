/* global localStorage, location, document, window, alert, encodeURIComponent, Date, JSON */
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      // Endpoint de login (método POST con body JSON)
      const LOGIN_URL = '/usuarios/login'

      // Realiza la petición al backend con credenciales en el body
      ...
      if (!resp.ok) {
        if (resp.status === 404) { alert('Usuario no encontrado') } else if (resp.status === 401) { alert('Correo o contraseña incorrectos') } else { alert(`Error del servidor: ${resp.status}`) }
