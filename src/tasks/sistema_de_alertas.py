from services.usuarios_service import service_obtener_usuario_para_frontend, obtener_clima_guardado
...
def analizar_condiciones_adversas(correo):
    """
    Analiza condiciones climáticas adversas comparando clima guardado vs actual.
    ...
    for cultivo in usuario.get("cultivos", []):
        # El cultivo puede tener "nombre" o "nombre_cultivo" dependiendo de la fuente
        ...
        if not nombre_cultivo:
            print(f"    ⏭️  Saltando - sin nombre")
            continue
        clima_guardado_response = obtener_clima_guardado(correo, nombre_cultivo)
        
        print(f"    📊 Clima guardado response: {list(clima_guardado_response.keys())}")

        if clima_guardado_response.get("error"):
            print(f"    ⏭️  Saltando - error: {clima_guardado_response.get('error')}")
            continue
        clima_guardado = clima_guardado_response.get("clima", {})
        if not clima_guardado:
            print(f"    ⏭️  Saltando - clima vacío")
            continue
        
        print(f"    ✅ Clima guardado encontrado")
        if not clima_guardado:
            continue
        clima_inicial = None
        ...
        temp_max_inicial = clima_inicial.get("max") or clima_inicial.get("temp_max") or 0
        temp_min_inicial = clima_inicial.get("min") or clima_inicial.get("temp_min") or 0
        ...
        estados_lluvia = ["rain", "drizzle", "thunderstorm", "lluvia", "tormenta"]
        llueve_ahora = any(estado in estado_actual for estado in estados_lluvia)
        lloveria_inicial = any(estado in estado_inicial for estado in estados_lluvia) or precipitacion_inicial > 0
        
        # DEBUG: Imprimir valores
        print(f"\n🔍 DEBUG - Cultivo: {nombre_cultivo}")
        print(f"   Clima inicial: max={temp_max_inicial}, min={temp_min_inicial}, estado='{estado_inicial}'")
        print(f"   Clima actual:  max={temp_max_actual}, min={temp_min_actual}, estado='{estado_actual}'")
        print(f"   Llueve ahora: {llueve_ahora}, Llovería inicial: {lloveria_inicial}")
        print(f"   Diferencia temp max: {temp_max_actual - temp_max_inicial}°C")
        print(f"   Diferencia temp min: {temp_min_inicial - temp_min_actual}°C")
        ...
        if temp_max_inicial and temp_max_actual > temp_max_inicial + 10:
            alertas.append({
                "cultivo": nombre_cultivo,
                "tipo_alerta": f"🌡️ Temperatura máxima más alta (+{round(temp_max_actual - temp_max_inicial, 1)}°C)",
                "severidad": "alta",
                "clima_inicial": {
        ...
        if temp_min_inicial and temp_min_actual < temp_min_inicial - 10:
            alertas.append({
                "cultivo": nombre_cultivo,
                "tipo_alerta": f"❄️ Temperatura mínima más baja ({round(temp_min_inicial - temp_min_actual, 1)}°C menos)",
                "severidad": "alta",
                "clima_inicial": {
        ...
        # 4. VIENTO FUERTE NO PRONOSTICADO (>30 km/h)
        viento_fuerte_actual = "wind" in estado_actual or "windy" in estado_actual
        ...
        if viento_fuerte_actual and not viento_fuerte_inicial:
            alertas.append({
                ...
                "severidad": "media",
                "clima_inicial": {
                    "viento": f"{viento_inicial} m/s" if viento_inicial else "Desconocido",
                    "estado": estado_inicial
                },
                "clima_actual": {
                    "estado": estado_actual,
        ...
        estados_malos = ["thunderstorm", "rain", "drizzle", "snow", "tormenta", "lluvia", "nieve"]
        
        era_bueno = any(estado in estado_inicial for estado in estados_buenos)
        es_malo_ahora = any(estado in estado_actual for estado in estados_malos)
        
        if era_bueno and es_malo_ahora:
            alertas.append({
                "cultivo": nombre_cultivo,
                "tipo_alerta": "⛈️ Cambio drástico: Clima favorable → Condiciones adversas",
                "severidad": "alta",
                "clima_inicial": {
                    "estado": estado_inicial,
